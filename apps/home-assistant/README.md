...
| Use Case | Description |
|----------|-------------|
| Fumoi "Cat Litter Box M4" (`localtuya`) | Generic **Tuya** device controlled fully on-LAN via **LocalTuya** ([`xZetsubou/hass-localtuya`](https://github.com/xZetsubou/hass-localtuya) pinned `2025.11.0`), installed via the `install-localtuya` init container. Discovered: Tuya category `msp`, protocol **v3.5**, IP `192.168.1.215`. Exposes `switch` (power), `select` (work mode), two `button`s (start / manual-clean), `number` (clean delay), and `cat_weight` / `excretion_*` / `work_state` / `use_time` / `status` sensors. |
...
| Config entry | Seeded by the `seed-localtuya` init container as a `no_cloud` hub entry (`localtuya` domain, `ENTRIES_VERSION=4`). Unlike Bambu/Tapo (flat 3-field entries) it's a **nested, versioned structure with a per-DP entity map**, authored against `config_flow.py` @ 2025.11.0 (per-platform required keys) + validated locally, not blind-seeded. The seed is **version-gated** (`SEED_VERSION` marker at `/config/.localtuya_seed_version`): bump it to re-seed a corrected map — the init container drops the stale entry and purges its entity-registry rows so old slugs don't linger. |
| DP labels | **Corrected 2026-07-03** against the authoritative Tuya cloud device spec (`/v1.0/devices/{id}/specifications` + shadow properties). The original guesses were wrong: DP6 = `cat_weight` (not "Counter A"), DP7 = `excretion_times_day`, DP8 = `excretion_time_day` (**not "Litter Level"** — there is no litter-level DP), DP9 = `manual_clean` command → **button** (not a "Bin Full" sensor — none exists), DP110 = `data_identification` (cat-presence status). Added the real runtime DPs: DP108 `work_state`, DP103 `use_time`. |
| ⚠️ Gotcha | `localtuya/__init__.py` reads `region`/`client_id`/`client_secret`/`user_id` **unconditionally** (before the `no_cloud` branch), so the seed includes those keys empty — omit them and setup `KeyError`s. Still live-correlate: DP1 `switch` reads `false` while the unit operates (likely an auto-run enable, not mains power) and the DP108 `work_state` / DP110 `data_identification` enum codes are unlabeled — toggle each in Smart Life and re-poll to map the enum values. `factory_reset` (DP23) is omitted (destructive). |
