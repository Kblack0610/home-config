...
- **`*.kblab.me` is LAN-only by convention.** Adding a new subdomain is an AdGuard rewrite (already wildcard-covered) + an Ingress with `host: <name>.kblab.me`. To expose something *publicly* instead, use a public zone (kennethblack.me etc.) and just add a Traefik Ingress — external-dns + the tunnel's single wildcard catch-all do the rest. See [`docs/public-sites.md`](./public-sites.md) (canonical). No tunnel edit or Terraform/DNS change is involved anymore.
