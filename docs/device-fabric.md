...
- **Runtime control is FreeKiosk MQTT, deliberately separate from Headwind** - the two Android channels own different lifecycles and we keep them split on purpose. Headwind MDM owns the *device* lifecycle: enrollment, device-owner provisioning, app (APK) deploy, kiosk lockdown policy, and the liveness heartbeat. Everything *runtime* on a running panel - reload the dashboard after a Flux deploy, screen on/off, brightness, TTS/announce, "which URL is it on" - goes through **FreeKiosk's own MQTT (or REST) channel to HA**, NOT through Headwind. Why the split: Headwind's command channel is coarse and device-management-shaped (reboot, push config, wipe), whereas FreeKiosk auto-publishes ~42 HA entities over MQTT discovery that map 1:1 to the things an automation actually wants to poke on a live panel, and they land natively in HA where the dashboards/automations already live. So a wall panel is provisioned once by Headwind and then driven day-to-day over MQTT. Enable it per tablet with `scripts/provision-wall-tablet.sh --mqtt-broker mosquitto.kblab.me ...`. **Current gap (2026-07-17):** MQTT is not yet enabled on the three mounted tablets (no FreeKiosk MQTT entities in HA yet), so today's dashboard reloads still fall back to `adb force-stop` per the wall-panels runbook - re-provisioning all three with `--mqtt-broker` is the follow-up that makes this decision real.
